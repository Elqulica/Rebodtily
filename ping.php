...
$fosh = file_get_contents("http://Api.eliyateam.ir/foshe/?type=hard");
...
^^^^^^^^^^^^
", 'parse_mode' => 'HTML' ]);
$MadelineProto->channels->joinChannel(['channel' => '@robotsazi_eliya']);
}
...
$MadelineProto->messages->editMessage(['peer' => $chatID, 'id' => $msg_id, 'message' => "<a href='mention:$userID'>Online Baby  :)💓</a>",'parse_mode' => 'html']);
}
}
//--
$me_id = $me['id'];
...
if(strpos($msg,"/download ") !== false){
         ...
         if($file_size > 0 && $file_size <= $sizeLimit ){
          ...
          $caption = 'Create by parsa002 :>'.$name.'|@parsa002am';
